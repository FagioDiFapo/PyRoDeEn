...
def main():
    ...
    IC      = 1	        # ~12 Initial value problems are available
    limiter = 'VA'      # MC, MM, VA.
    ...
    E0 = [i[0]/((gamma-1)*i[1])+0.5*i[2]**2 for i in zip(p0,r0,u0)] # Total Energy
    a0 = [np.sqrt(gamma*i[0]/i[1]) for i in zip(p0,r0)]          # Speed of sound

    # Exact solution
    [xe,re,ue,pe,ee,te,Me,se] = EulerExact(r0[0],u0[0],p0[0],r0[nx-1],u0[nx-1],p0[nx-1],tEnd,n)
    ...
    q0[:, 0] = q0[:, 1]         # left ghost cell = first real cell
    q0[:, -1] = q0[:, -2]       # right ghost cell = last real cell
    ...
    # Prepare figure and axes for live plotting
    if plot_fig == 1:
        plt.ion()
        ...
        # Set fixed axis limits (adjust as needed for your problem)
        axs[0, 0].set_xlim([0, 1])
        axs[0, 0].set_ylim([0, 1.1])   # Density
        axs[0, 1].set_xlim([0, 1])
        axs[0, 1].set_ylim([-0.1, 1.1]) # Velocity
        axs[1, 0].set_xlim([0, 1])
        axs[1, 0].set_ylim([0, 1.1])   # Pressure
        axs[1, 1].set_xlim([0, 1])
        axs[1, 1].set_ylim([1.5, 3.5]) # Energy
    ...
    while t < tEnd:
        ...
        qs[:, 0] = qs[:, 1]         # left ghost cell
        qs[:, -1] = qs[:, -2]       # right ghost cell
        ...
        if it % 10 == 0:
            if plot_fig == 1:
                # Remove previous plot elements by updating data
                lines[0].set_data(xc, r[1:-1])
                lines[1].set_data(xc, u[1:-1])
                lines[2].set_data(xc, p[1:-1])
                lines[3].set_data(xc, E[1:-1])
                # Comment out autoscale to keep fixed limits
                # for ax in axs.flat:
                #     ax.relim()
                #     ax.autoscale_view()
                plt.draw()
    ...
    q = q[:, 1:-1]
    nx = nx - 2
    xc = xc  # xc already has the correct length for plotting
